...
### 2026-09-22 — Ad hoc fix: playlist name optional in Standard mode

Not tied to a numbered session — Dre flagged that Standard mode shouldn't require a playlist name before Create Room. Checked `server/dynamic-songs.js`'s `getDynamicPool()` first: it currently ignores the name entirely and just returns a shuffled static 50-artist pool regardless of input, so the requirement wasn't protecting any real functionality in Standard mode. Relaxed the `hostGame()` validation in `client/src/components/HomeScreen.jsx` to only require the name when `gameMode !== 'standard'`, and updated the field's hint text to say "(optional)" when Standard is selected. Newlywed, Gong Show, and DJ Battle still require it — DJ Battle's block in particular surfaces the name directly to players. Committed as `5142428`, pushed, deployed to Railway (confirmed SUCCESS). Dre noted this may become more useful in the future (implying pool generation may eventually actually use the playlist name) — worth remembering if `dynamic-songs.js` is ever revisited.

### 2026-09-22 — Bug fix: manual "mark as played" screen was completely non-functional

Dre reported the Android Join Room button doing nothing (separate, still-open investigation below), and along the way flagged that the Host controls screen said "Click a song to mark it as played" even though song mode is retired. Investigating that surfaced a real, bigger bug: **the manual click-to-mark UI (for Manual music source, and the "didn't detect it" fallback under Spotify/AudD sources) was rendering zero buttons, ever** — it iterates `room.songPool`, which has been permanently empty (`[]`) since song mode was retired (see `server/game.js`'s `createRoom`). Even if it had rendered anything, `host:play_song`'s server handler only ever passed `songTitle` through, hardcoding `songArtist: null`, so a click would have hit `'Song not found'` anyway.

**Fixed:**
- `client/src/components/GameScreen.jsx`: new shared `ManualMarkGrid` component replaces all three duplicated dead-songPool blocks (Manual mode's primary marking UI, Spotify's fallback, AudD's fallback). It lists every artist any confirmed player predicted (same aggregation pattern as Session 3's Selection Summary), and clicking one emits `host:play_song` with both `songTitle` and `songArtist` set to the artist name. Copy updated from "song" to "artist" throughout (button label, fallback hints, summary text). The "Played:" chip list also no longer shows the redundant `ArtistName — ArtistName`.
- `server/index.js`: `host:play_song` now actually reads and passes `songArtist` through to `game.playSong` instead of hardcoding `null`.
- Closed a related observability gap found while debugging: `player:join`'s duplicate-name and room-not-found rejections, and `player:rejoin`'s failure path, previously emitted client-side errors with **no server-side log line at all** — only unexpected exceptions were logged (Session 2's `wrapHandler`). Added `console.warn` on all three graceful-rejection paths.

**Verified via scripted socket tests against a genuinely fresh local dev server** (see environment note below): host manually marks a predicted artist as played → correctly matches the player's pick, scores it, and the game can be won this way. Duplicate-name and bad-room-code join attempts now produce a server-side log line. `npm run build` clean.

**Environment issue discovered and cleaned up (not a code bug, but wasted real debugging time):** found 7+ orphaned `node`/`vite` processes for this project alone, some dating back to Sept 20, still bound to ports 3009/5174 — leftover from `start.sh stop` not reliably killing everything (matches the known PID-capture flakiness noted in `~/.claude/CLAUDE.md`'s Coherence Suite section, evidently not unique to those apps). This caused a real false negative mid-session: an initial test of the manual-marking fix hit a stale zombie process running old code and failed, before the actual fresh-server test confirmed the fix works. Killed all stale pandora-bingo processes and confirmed a clean single instance before re-testing. **`start.sh` fixed** (Dre approved): applied the same subshell-exec launch pattern already used in Codex's and Hub's `start.sh`. The frontend launch now calls `./node_modules/.bin/vite` directly instead of `npm run dev` — npm was the actual culprit, since `$!` was capturing npm's PID, not vite's, so killing it on `stop` didn't reliably take vite (and its esbuild child) down with it. Both backend and frontend launches now use `(cd DIR && exec CMD) &` so `$!` is always the real process. `stop` also gained a `fuser -k <port>/tcp` fallback on both ports as a safety net, and output now goes to `/tmp/pandora.log` with a new `bash start.sh logs` command. Verified: start → both recorded PIDs are the real `node`/`vite` processes (no wrapper) → stop → both ports free, zero orphans, and none of the other Coherence Suite apps' processes (ptm/goalkeeper/manifest/assemble/masterplan/codex, all confirmed running normally throughout) were touched.

**Still open:** the original Android "Join Room button does nothing" report. Railway logs show the Android device's socket *did* reach the server this time (unlike the earlier Tailscale mix-up), connecting right after a room was created, but no `player:join` ever landed server-side for it — and no error either. Root cause not yet found; needs either a repro with the new join-failure logging in place, or Dre confirming exactly what he saw on screen when tapping the button.
